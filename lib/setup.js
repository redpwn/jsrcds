...
const getDiff = (...paths) => new Promise((resolve) => {
...
await core.group('builds', () => Promise.all(challenges.flatMap(challenge =>
  Object.entries(challenge.containers).map(async ([name, container]) => {
    if (!container.build) {
      return
    }
    const context = path.posix.join(challenge.segment, container.build.context)
...
await core.group('uploads', () => Promise.all(uploads.map(async upload => {
  if (!upload.filePath) {
    return
  }
